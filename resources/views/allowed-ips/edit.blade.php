    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-xl p-6">
                <h2 class="text-2xl font-bold text-gray-800 mb-2">Edit Allowed IP</h2>
                <p class="text-sm text-gray-500 mb-6">
                    Update IP access rule details.
                </p>
                ...
                <form method="POST" action="{{ route('allowed-ips.update', $allowedIp) }}" class="space-y-5">
                    @csrf
                    @method('PUT')
                        ...
                        <select name="user_id"
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->id }}"
                                    {{ (string) old('user_id', $allowedIp->user_id) === (string) $user->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text"
                               name="label"
                               value="{{ old('label', $allowedIp->label) }}"
                    ...
                    <div class="flex items-center gap-2">
                        <input type="checkbox"
                               name="is_active"
                               value="1"
                               {{ old('is_active', $allowedIp->is_active) ? 'checked' : '' }}
