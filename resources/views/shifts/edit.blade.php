    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-xl p-6">
                <h2 class="text-2xl font-bold text-gray-800 mb-2">Edit Shift</h2>
                <p class="text-sm text-gray-500 mb-6">Update shift timing, break timing and grace minutes.</p>
                ...
                <form method="POST" action="{{ route('shifts.update', $shift) }}" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="time" name="break_start_time" value="{{ old('break_start_time', $shift->break_start_time ? \Carbon\Carbon::parse($shift->break_start_time)->format('H:i') : '') }}"
                                   class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Break End Time</label>
                            <input type="time" name="break_end_time" value="{{ old('break_end_time', $shift->break_end_time ? \Carbon\Carbon::parse($shift->break_end_time)->format('H:i') : '') }}"
                                   class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" min="0" name="grace_minutes" value="{{ old('grace_minutes', $shift->grace_minutes) }}"
                               class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
